...
Данный модуль обеспечивает взаимодействие между интернет-магазином на базе CMS **CS-Cart** и сервисом платежей [ХуткiГрош](www.hutkigrosh.by).
...
1. Загрузите архив с модулем [cs-cart-hutkigrosh-module.zip](https://bitbucket.org/esasby/cmsgate-cscart-hutkigrosh/raw/master/cs-cart-hutkigrosh-module.zip)
...
1. Перейдите в меню `Администрирование - Способы оплаты `. В списке появятся два новых пункта:
    * Через систему «Расчет» (ЕРИП) - содержит основные настройки плагина
    * Оплата картой
1. Перейдите к редактированию способа оплаты "Через систему «Расчет» (ЕРИП)". Заполните параметры для идентификации вашего магазина в системе ХуткiГрош
...
* Для автоматического обновления статуса заказа (после оплаты клиентом выставленного в ЕРИП счета) необходимо сообщить в службу технической поддержки сервиса «Хуткi Грош» адрес обработчика:
    ...
    ```
* Модуль ведет лог файл по пути _app/addons/hutkigrosh/lib/vendor/esas/cmsgate-core/logs/cmsgate.log_
